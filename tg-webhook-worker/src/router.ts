...
import { handleAuthCheck } from "./handlers/authcheck";
...
export async function route(env: any, update: any) {
  ...
  if (text === "/logout") return await handleLogout(env, update);
    if (text === "/authcheck") return await handleAuthCheck(env, update);
if (text === "/session") return await handleSession(env, update);

  
  if (text === "/debug") return await handleDebug(env, update);// public
